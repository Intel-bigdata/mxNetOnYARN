...
def submit(args):
    ...
    curr_path = os.path.dirname(os.path.abspath(os.path.expanduser(__file__)))
    YARN_BOOT_PY = os.path.join(curr_path, 'launcher.py')
    if args.native != 'python':
        print args.command
        if args.command[0] == 'python':
            # Extract python command from args.command
            python_job = " ".join(args.command[:2])
            # TODO Call python_job and save all related parameters and symbols
            ...
            if args.native == 'cpp':
                ...
                pass
            if args.native == 'java':
                # TODO after JAVA interface is complete
                pass
            print args.command
